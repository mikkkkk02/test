				...
				<div class="col-sm-12">
					...
					<div class="row">
						<div class="form-group col-sm-3">
							<label>Year</label>
							<select v-model="filter.year" @change="fetch()"
							class="form-control">

								<template v-for="(year, index) in years">
									
									<option :value="year" :selected="index == 1">{{ year }}</option>
						...
						<div class="form-group col-sm-3">
							<label>Learning Act. Type</label>
							<select v-model="filter.learningtype" @change="fetch()"
							class="form-control">

								<option value="x">Choose here...</option>

								<template v-for="learningtype in learningtypes">
									
									<option :value="learningtype['value']">{{ learningtype['label'] }}</option>
						...
						<div class="form-group col-sm-3">
							...
							<select v-model="filter.competencytype" @change="fetch()"
							class="form-control">

								<option value="x">Choose here...</option>

								<template v-for="competencytype in competencytypes">
									
									<option :value="competencytype['value']">{{ competencytype['label'] }}</option>
						...
						<div class="form-group col-sm-3">
							...
							<select v-model="filter.specificcompetency" @change="fetch()"
							name="" class="form-control">

								<option value="x">Choose here...</option>

								<template v-for="specificcompetency in specificcompetencies">
									
									<option :value="specificcompetency.id">{{ specificcompetency.name }}</option>
								...
								<tbody>
									...
									<template v-for="(list, index) in lists">
										...
										<tr>
											...
											<td>{{ truncate(list.details, 75) }}</td>
											<td><center>{{ list.completion_year }}</center></td>
											<td>
												<center>
													<span :class="{
														'bg-yellow': isStatus('ongoing', list.status),
														'bg-orange': isStatus('for completion', list.status),
														'bg-green': isStatus('completed', list.status),
													}"
		...
		methods: {
			...
			*/
			...
			getURL: function(url) {
				...
				if(this.filter.year != 'x')
					url = this.addURLParam('year', this.filter.year, url);

				if(this.filter.learningtype != 'x')
					url = this.addURLParam('learningtype', this.filter.learningtype, url);

				if(this.filter.competencytype != 'x')
					url = this.addURLParam('competencytype', this.filter.competencytype, url);

				if(this.filter.specificcompetency != 'x')
					url = this.addURLParam('specificcompetency', this.filter.specificcompetency, url);
