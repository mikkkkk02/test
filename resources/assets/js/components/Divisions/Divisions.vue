	...
	>
		...
		<thead slot="thead">
			<tr>
				...
				<th><center>No. of Departments</center></th>
			...
			<tr>
				...
				<td v-if="!settings.nocompany">
					...
					<template v-if="list.company">
						...
						<a :href="list.company.extra.view">{{ list.company.name }}</a>					
	
					</template>

				</td>
				<td><center>{{ list.extra.departments }}</center></td>
				<td>
					<center>
	...
	export default {
		...
		methods: {
			...
			init: function() {
				...
				this.settings.nocompany = this.nocompany || false;
			},
