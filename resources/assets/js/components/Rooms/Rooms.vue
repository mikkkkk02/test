	...
	>
		...
		<thead slot="thead">
			<tr>
				...
				<th>
					<center slot="column">Location</center>
				</th>

				<th>
					<center slot="column">Color</center>
				</th>
		...
		<template slot="data" slot-scope="{list}">
			...
			<tr>
				...
				class="checkbox">
					<input :value="list.id"
					type="checkbox" name="rooms[]">
				</td>				
				<td>{{ list.id }}</td>
				<td>{{ list.name }}</td>
				<td>{{ list.location.name }}</td>
				<td><span class="btn-xs border" :style="'background:#' + list.color + '; border: 1px solid #333;'">{{ list.color }}</span></td>
				<td>
					<center>
