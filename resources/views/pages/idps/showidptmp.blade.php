...
@section('breadcrumb')
	...
	<div class="content-header">
		<h1>Update to IDP # {{ $idp->id }}<small>Requested by {{ $idp->updater->renderFullname() }}</small></h1>
	    <ol class="breadcrumb">
	        <li>
		...
		<div class="col-sm-12">
			...
			<div class="row">
				<div class="col-sm-12 l-margin-b">
					...
					@if($self->isIdpApprover($idp->idp->id))
					<form id="approveTempIDPForm" method="post" action="{{ route('idp.approve', $idp->idp->id) }}" data-redirect="true" class="ajax inline">
					...
					<form id="disapproveTempIDPForm" method="post" action="{{ route('idp.disapprove', $idp->idp->id) }}" data-redirect="true" class="ajax inline">
